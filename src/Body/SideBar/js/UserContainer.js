...
export default function UserContainer() {
  ...
  return (
    <div className={styles.divUserContainer}>
      ...
      {arrayDiscovers.map((discover) => 
        {
            const convertDiscover = Object.values(discover)[0]
            return(
                <div key={convertDiscover.id} className={styles.divUserLinkContainer}>
                    <a href={convertDiscover.link} >
                        <div className={styles.divUserAvatar} style={{width:"32px",height:"32px"}}>
                            <span className={styles.spanUserAvatar} style={{width:"32px",height:"32px"}}>
                                <img src={convertDiscover.cover} className={styles.imgAvatar}/>
                            </span>
                        </div>
                    </a>
                    <a href={convertDiscover.link} className={styles.userContentLink} style={{textDecoration:"none"}}>
                        <div className={styles.divUserTitleWrapper}>
                            <h4 className={styles.h4UserTitle}>{convertDiscover.subTitle.slice(1)}</h4>
                            <div>
                                <IconVerify />
                            </div>
                        </div>
